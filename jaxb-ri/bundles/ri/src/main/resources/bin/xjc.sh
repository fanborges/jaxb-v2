...
VERSION_STRING=`${JAVA} -version 2>&1 | head -n 1 | sed -E 's/.+version "(.+)"/\1/'`
JAVA_VERSION=`echo ${VERSION_STRING} | sed -E 's/^(1\.)?([0-9]+).+$/\2/'`
...
then
  ...
  #module path
  exec "${JAVA}" --module-path "${JAXB_PATH}" --add-reads com.sun.xml.xsom=relaxngDatatype ${XJC_OPTS} -m com.sun.tools.xjc/com.sun.tools.xjc.XJCFacade "$@"
fi
